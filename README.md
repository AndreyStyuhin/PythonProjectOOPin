...
Общая функциональность, описана в абстрактном классе.

    #ABC    #abstractmethod

Реализован класс-миксин, в работе метода __init__, выводящий в консоль информацию о том, от какого класса и с какими параметрами был создан объект.
...
При инициализации экземпляра класса Product происходит обработка попытки создания товара с нулевым количеством, если пользователь создает товар (метод __init__) с нулевым количеством, происходит вызов исключения ValueError, которое выдает сообщение: 
Товар с нулевым количеством не может быть добавлен. При этом прерывается выполнение программы.

    #raise

В классе Category реализован новый метод, который подсчитывает средний ценник всех товаров. С помощью исключений обрабатывается случай, когда в категории нет товаров и сумма всех товаров будет делиться на ноль. Если такое происходит, возвращается ноль.
